...
player.dash.SegmentIndex.prototype.getOffset = function(segNum) {
  return segNum > this.count_ ? null : this.offsets_[segNum];
};
...
player.dash.SegmentIndex.prototype.findForTime = function (time) {
  var idx = this.count_; // MWMWMW Changed from this.count_ - 1
  ...
  //log("Found " + idx.toString() + " for time " + time.toString());
  return idx;
};
...
player.dash.SegmentIndex.prototype.addSegmentBySize =
    function (length, duration) {
      ...
      this.startTimes_[this.count_] = Math.floor(this.offsets_[this.count_] / 1e4) / 1e3;//this.startTimes_[this.count_ - 1] + duration;
      //console.log('adding offset at ' + this.startTimes_[this.count_] + '. t = ' + this.offsets_[this.count_]);
    };
...
player.dash.SegmentIndex.prototype.populateIdx = function (chunks) {
  //if (this.count_ < 5) {
  this.setFirstSegmentStart(chunks[0].t, chunks[0].t / 1e7);
    for (var i = 0; i < chunks.length-1; i++) {
      this.addSegmentBySize(chunks[i].d, chunks[i].d / 1e7);
    }
  //}
};
...
player.utils.WebMElemParser_.prototype.readCodedInt_ = function (useMask) {
  var value = this.readByte_(), i = null;
